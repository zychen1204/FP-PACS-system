...
#
# 自動化選項：
#   AUTO_CLOUD_SEED=1  部署後自動執行 90,000 人 cloud seed Job
#   AUTO_LOAD_TEST=1   部署後自動執行 PostgreSQL 歷史刷卡壓測匯入 Job
#
# 前置條件：
...
AUTO_CLOUD_SEED=${AUTO_CLOUD_SEED:-0}
AUTO_LOAD_TEST=${AUTO_LOAD_TEST:-0}
CLOUD_SEED_TIMEOUT=${CLOUD_SEED_TIMEOUT:-20m}
LOAD_TEST_TIMEOUT=${LOAD_TEST_TIMEOUT:-60m}
...
echo "║ Seed  : AUTO_CLOUD_SEED=$AUTO_CLOUD_SEED"
echo "║ Load  : AUTO_LOAD_TEST=$AUTO_LOAD_TEST"
echo "╚════════════════════════════════════════════════════════╝"
...
# ── 0. Billing preflight ──────────────────────────────────────
echo ""
echo "💳 [0/7] 檢查 Project Billing 狀態..."
BILLING_ENABLED=$(gcloud billing projects describe "$PROJECT_ID" \
    --format="value(billingEnabled)" 2>/dev/null || echo "False")
if [ "$BILLING_ENABLED" != "True" ] && [ "$BILLING_ENABLED" != "true" ]; then
    echo "❌ Project '$PROJECT_ID' 尚未啟用 Billing，無法建立 GKE / Cloud SQL / Memorystore。"
    echo "   請先到 Google Cloud Console 綁定 Billing account 後再重跑："
...
echo "   building pacs-load-generator ..."
docker build -t "gcr.io/$PROJECT_ID/pacs-load-generator:latest" ./scripts/load-generator
docker push "gcr.io/$PROJECT_ID/pacs-load-generator:latest"
echo "   ✅ 所有 Images 推送完成"
...
run_job_and_stream_logs() {
    local job_name=$1
    local timeout=$2

    echo "   ⏳ 等待 $job_name Pod 建立..."
    ...
    echo "   📜 $job_name logs:"
    kubectl logs -f "job/$job_name" --namespace=pacs || true

    echo "   ⏳ 等待 $job_name 完成（timeout: $timeout）..."
...
echo ""
echo "2. Ingress 狀態（IP 可能需 2-3 分鐘才分配）："
kubectl get ingress pacs-ingress -n pacs || true
echo ""

if [ "$AUTO_CLOUD_SEED" = "1" ]; then
    echo "🌱 AUTO_CLOUD_SEED=1：執行 90,000 人 cloud seed..."
    kubectl delete job pacs-cloud-seed -n pacs --ignore-not-found
    kubectl apply -f k8s/12-cloud-seed.yaml
    run_job_and_stream_logs pacs-cloud-seed "$CLOUD_SEED_TIMEOUT"
    echo "   ✅ pacs-cloud-seed 完成"
else
    echo "3. 雲端大規模播種（90,000 人，手動執行或設 AUTO_CLOUD_SEED=1 自動執行）："
    echo "   kubectl delete job pacs-cloud-seed -n pacs --ignore-not-found"
    echo "   kubectl apply -f k8s/12-cloud-seed.yaml"
    echo "   kubectl logs -f job/pacs-cloud-seed -n pacs"
fi
echo ""

if [ "$AUTO_LOAD_TEST" = "1" ]; then
    echo "🔥 AUTO_LOAD_TEST=1：執行 PostgreSQL 歷史刷卡資料壓測匯入..."
    ...
    run_job_and_stream_logs pacs-load-test "$LOAD_TEST_TIMEOUT"
    echo "   ✅ pacs-load-test 完成"
else
    echo "4. PostgreSQL 歷史刷卡資料壓測匯入（手動執行或設 AUTO_LOAD_TEST=1 自動執行）："
    echo "   kubectl delete job pacs-load-test -n pacs --ignore-not-found"
    echo "   sed 's|gcr.io/PROJECT_ID|gcr.io/$PROJECT_ID|g' k8s/07-load-tester.yaml | kubectl apply -f -"
    echo "   kubectl logs -f job/pacs-load-test -n pacs"
fi
echo ""
