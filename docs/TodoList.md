...
## 3. 資料庫 (Database)
- [ ] **大規模模擬樣本 (0104)**：利用後端修改後的內容（HTTP POST 帶時間戳）生成大量刷卡紀錄，產出滿足 HW2 規格（本地/雲端）。可參考 [SimulationGuide.md](SimulationGuide.md) 但必須改成使用 **HTTP POST** 方式進行模擬(利用後端更改支援模擬時間戳 (0103)內容)以測試真實 API 壓力，也可再加上K6做補充純負載測試。
