-- ============================================================
-- 0106_mv_exclude_future.up.sql
--
-- Defense-in-depth：mv_daily_attendance 硬性排除未來事件。
--
-- 動機：
--   * seed 紀律已修（0099 + seed-generator 不再寫未來），但仍可能因為
--     - timezone bug
--     - 測試誤插入未來時間
--     - event-processor consumer lag 在 catch-up 時誤打 future timestamp
--     導致 MV 內混入 event_time > NOW() 的列。
--   * 一旦混入，所有 attendance/trend 報表都會被汙染。
--   * 在 source CTE 加 `event_time <= NOW()`，從 MV 層阻斷。
--
-- 演算法繼承 0105，僅在 events_with_counter / per_day_summary 加未來時間 cap。
-- DROP + CREATE 而非 ALTER（PG 不支援 ALTER MV 改 SELECT）。
-- ============================================================
...
WITH events_with_counter AS (
    ...
    WHERE e.status = 'SUCCESS'
      AND e.event_time <= NOW()  -- 0106: 硬性排除未來事件
),
visit_start AS (
    SELECT
        badge_id,
...
per_day_summary AS (
    ...
    FROM access_events e
    WHERE e.status = 'SUCCESS'
      AND e.event_time <= NOW()  -- 0106: 同步在這層 cap
